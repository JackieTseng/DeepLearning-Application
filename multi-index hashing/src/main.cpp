#include "mih.h"

int main(int argc, const char *argv[]) {
    //double _1_, _2_, _3_, _4_, _5_, _6_;//win
    clock_t _1_, _2_, _3_, _4_, _5_, _6_;//unix
    string target = "";
    //string target = "0011101110000010010011111001111110001110001010001110100010001011";//378
    _1_ = clock();
    readData("B_1000_25w.txt");
    _2_ = clock();
    //readData("B_64.txt");
    calculateVariance();
    //readDataAndChange("B_1000_25w.txt");
    //readDataAndChange("B_64.txt");
    MIH x = MIH(HASHTABLENUMBER);
    _3_ = clock();
    bitset<BITWIDTH> t(target);
    //bit_transform(t);
    _4_ = clock();
    set<int> candidates;
    candidates = x.searchCandidates(t, RADIUS, candidates);
    _5_ = clock();
    set<int> result;
    result = x.selectGoal(candidates, t, RADIUS, result);
    _6_ = clock();
    cout << "result : " << candidates.size() << " -> " << result.size() << endl;
    /*win*/
    //cout << "Bulid time           : " << _2_ - _1_ << endl;
    //cout << "Find Candidate  time : " << _3_ - _2_ << endl;
    //cout << "Find Goal time       : " << _4_ - _3_ << endl;
    //cout << "Total time           : " << _4_ - _1_ << endl;
    /*unix*/
    cout << "Read time            : " << (float)(_2_ - _1_)/CLOCKS_PER_SEC << endl;
    cout << "Bulid time           : " << (float)(_3_ - _2_)/CLOCKS_PER_SEC << endl;
    cout << "Transform  time      : " << (float)(_4_ - _3_)/CLOCKS_PER_SEC << endl;
    cout << "Find Candidate  time : " << (float)(_5_ - _4_)/CLOCKS_PER_SEC << endl;
    cout << "Find Goal time       : " << (float)(_6_ - _5_)/CLOCKS_PER_SEC << endl;
    cout << "Total time           : " << (float)(_6_ - _1_)/CLOCKS_PER_SEC << endl;
    cout << "finish\n";
    return 0;
}
